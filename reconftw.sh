...
function start(){
	...
	if [ -z $BBRF_PROGRAM ]; then
		program_bbrf="$BBRF_PROGRAM"
	...
	elif [ "$BBRF_CONNECTION" = true ]; then
...
function help(){
	...
	printf "   -b --bbrf 	     BBRF Program to store results under\n"
...
PROGARGS=$(getopt -o 'd:m:l:b:x:i:o:f:q:c:rspanwvh::' --long 'domain:,bbrf:list:,recon,subdomains,passive,all,web,osint,deep,help,vps' -n 'reconFTW' -- "$@")
